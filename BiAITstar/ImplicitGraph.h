...
class ImplicitGraph {
 public:
  ...
  // Return: vector of ptr that point to the meet valid edges which can not help
  // to improve current solution anymore;
  ...
  void setMaxNumberOfGoals(std::size_t maxNumberOfGoals) {
    maxNumberOfGoals_ = maxNumberOfGoals;
  }
  ...
  std::vector<std::shared_ptr<Vertex> > getNeighbors(
 ...
 private:
  ...
  // The radius that defines the neighborhood of a vertex if using an r-disc
  // graph;
  double radius_{std::numeric_limits<double>::infinity()};

  // The number of neighbors that defines the neighborhood of a vertex if using
  // a k-nearest graph;
  std::size_t numNeighbors_{std::numeric_limits<std::size_t>::max()};

  // A constant for the computation of the number of neighbors when using a
  // k-nearest model;
  ...
  // The start vertices that have been pruned. They are kept around because if
  // the user decides to add goal states after we've pruned some start states,
  // we might want to add these pruned start states again;
  std::vector<std::shared_ptr<Vertex> > prunedStartVertices_{};

  // The goal vertices that have been pruned. They are kept around because if
  // the user decides to add start states after we've pruned some goal states,
  // we might want to add these pruned goal states again.
  ...
  std::vector<MeetValidEdgeQueue::Element *>
      vectorOfMeetValidEdgesToBePruned_{};
};
}  // namespace ompl::geometric::biait
