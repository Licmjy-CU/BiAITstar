...
std::vector<std::weak_ptr<Vertex>> Vertex::invalidateForwardValidBranch(
  ...
  std::vector<std::weak_ptr<Vertex>> accumulatedChildren =
      forwardValidChildren_;
...
std::vector<std::weak_ptr<Vertex>> Vertex::invalidateReverseValidBranch(
  ...
  for (const auto &elem : accumulatedChildren) {
    if (auto elemSharedPtr = elem.lock()) {
      ...
      auto elem_sAccumulatedChildren =
          elemSharedPtr->invalidateReverseValidBranch(
              vectorOfMeetValidEdgesToBePruned);  // s for shared_ptr;
      accumulatedChildren.insert(accumulatedChildren.end(),
                                 elem_sAccumulatedChildren.begin(),
                                 elem_sAccumulatedChildren.end());
...
std::vector<std::weak_ptr<Vertex>> Vertex::invalidateForwardLazyBranch() {
  std::vector<std::weak_ptr<Vertex>> accumulatedLazyChildren =
      forwardLazyChildren_;
  for (const auto &elem : accumulatedLazyChildren) {
    if (auto elemSharedPtr = elem.lock()) {
      ...
      auto elem_sAccumulatedChildren =
          elemSharedPtr->invalidateForwardLazyBranch();
...
std::vector<std::weak_ptr<Vertex>> Vertex::invalidateReverseLazyBranch() {
  ...
  for (const auto &elem : accumulatedLazyChildren) {
    if (auto elemSharedPtr = elem.lock()) {
      ...
      auto elem_sAccumulatedChildren =
          elemSharedPtr->invalidateReverseLazyBranch();
      accumulatedLazyChildren.insert(accumulatedLazyChildren.end(),
                                     elem_sAccumulatedChildren.begin(),
                                     elem_sAccumulatedChildren.end());
    }
...
void Vertex::addToForwardLazyChildren(const std::shared_ptr<Vertex> &vertex) {
  ...
  if (std::find_if(forwardLazyChildren_.begin(), forwardLazyChildren_.end(),
                   [vertex](const std::weak_ptr<Vertex> &child) {
...
void Vertex::removeFromForwardLazyChildren(std::size_t vertexId) {
  auto iter =
      std::find_if(forwardLazyChildren_.begin(), forwardLazyChildren_.end(),
...
bool Vertex::hasForwardLazyChild(const std::shared_ptr<Vertex> &vertex) const {
  return std::any_of(forwardLazyChildren_.begin(), forwardLazyChildren_.end(),
                     [&vertex](const std::weak_ptr<Vertex> &elem) {
                       return elem.lock()->getId() == vertex->getId();
                     });
}
...
void Vertex::removeFromReverseLazyChildren(std::size_t vertexId) {
  auto iter =
      std::find_if(reverseLazyChildren_.begin(), reverseLazyChildren_.end(),
                   [vertexId](const std::weak_ptr<Vertex> &child) {
                     return vertexId == child.lock()->getId();
                   });
...
void Vertex::removeFromForwardValidChildren(std::size_t vertexId) {
  auto iter =
      std::find_if(forwardValidChildren_.begin(), forwardValidChildren_.end(),
                   [vertexId](const std::weak_ptr<Vertex> &child) {
                     return vertexId == child.lock()->getId();
                   });
...
void Vertex::removeFromReverseValidChildren(std::size_t vertexId) {
  auto iter =
      std::find_if(reverseValidChildren_.begin(), reverseValidChildren_.end(),
                   [vertexId](const std::weak_ptr<Vertex> &child) {
                     return vertexId == child.lock()->getId();
                   });
...
bool Vertex::isConsistentInForwardLazySearch() const {
  return optObjPtr_->isCostEquivalentTo(heuristicCost_rhs_ForwardLazy_,
                                        heuristicCost_g_ForwardLazy_);
}

bool Vertex::isConsistentInReverseLazySearch() const {
  return optObjPtr_->isCostEquivalentTo(heuristicCost_rhs_ReverseLazy_,
...
void Vertex::setForwardValidParent(const std::shared_ptr<Vertex> &vertex,
...
void Vertex::setReverseValidParent(const std::shared_ptr<Vertex> &vertex,
                                   const base::Cost &edgeCost) {
...
void Vertex::removeFromValidQueueIncomingLookupFromStart(
    EdgeQueue::Element *pointer) {
  validQueueIncomingLookupFromStart_.erase(
      std::remove(validQueueIncomingLookupFromStart_.begin(),
                  validQueueIncomingLookupFromStart_.end(), pointer),
      validQueueIncomingLookupFromStart_.end());
}

void Vertex::removeFromValidQueueOutgoingLookupFromStart(
    EdgeQueue::Element *pointer) {
  validQueueOutgoingLookupFromStart_.erase(
      std::remove(validQueueOutgoingLookupFromStart_.begin(),
                  validQueueOutgoingLookupFromStart_.end(), pointer),
      validQueueOutgoingLookupFromStart_.end());
}

void Vertex::removeFromValidQueueIncomingLookupFromGoal(
    EdgeQueue::Element *pointer) {
  validQueueIncomingLookupFromGoal_.erase(
      std::remove(validQueueIncomingLookupFromGoal_.begin(),
                  validQueueIncomingLookupFromGoal_.end(), pointer),
      validQueueIncomingLookupFromGoal_.end());
}

void Vertex::removeFromValidQueueOutgoingLookupFromGoal(
    EdgeQueue::Element *pointer) {
  validQueueOutgoingLookupFromGoal_.erase(
      std::remove(validQueueOutgoingLookupFromGoal_.begin(),
                  validQueueOutgoingLookupFromGoal_.end(), pointer),
      validQueueOutgoingLookupFromGoal_.end());
}
...
bool Vertex::hasWhitelistedChild(const std::shared_ptr<Vertex> &vertex) const {
  // Check if the vertex is whitelisted by iterating over all whitelisted
  // children. It this detects an invalid vertex, e.g., a vertex that was once
  // whitelisted but has been pruned since, remove the vertex from the list of
...
void Vertex::setForwardLazyQueuePointer(
    typename VertexQueue::Element *pointer) {
  forwardLazyQueuePointer_ = pointer;
}

void Vertex::setReverseLazyQueuePointer(
    typename VertexQueue::Element *pointer) {
  reverseLazyQueuePointer_ = pointer;
}

void Vertex::addToValidQueueIncomingLookupFromStart(
    EdgeQueue::Element *pointer) {
  validQueueIncomingLookupFromStart_.emplace_back(pointer);
}

void Vertex::addToValidQueueOutgoingLookupFromStart(
    EdgeQueue::Element *pointer) {
  validQueueOutgoingLookupFromStart_.emplace_back(pointer);
}

void Vertex::addToValidQueueIncomingLookupFromGoal(
    EdgeQueue::Element *pointer) {
  validQueueIncomingLookupFromGoal_.emplace_back(pointer);
}

void Vertex::addToValidQueueOutgoingLookupFromGoal(
    EdgeQueue::Element *pointer) {
