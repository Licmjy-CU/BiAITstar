...
namespace ompl::geometric {
    ...
    BiAIT::BiAIT(const base::SpaceInformationPtr &spaceInformationPtr)
            : base::Planner(spaceInformationPtr, "BiAIT"), solutionCost_(), implicitGraph_(solutionCost_),
              forwardValidQueue_([this](const auto &lhs, const auto &rhs) { return isEdgeBetter(lhs, rhs); }),
              reverseValidQueue_([this](const auto &lhs, const auto &rhs) { return isEdgeBetter(lhs, rhs); }),
              meetValidEdgeQueue_([this](const auto &lhs, const auto &rhs) { return isMeetValidEdgeBetter(lhs, rhs); }),
              meetLazyEdgeQueue_([this](const auto &lhs, const auto &rhs) { return isMeetLazyEdgeBetter(lhs, rhs); }),
              forwardLazyQueue_([this](const auto &lhs, const auto &rhs) { return isVertexBetter(lhs, rhs); }),
              reverseLazyQueue_([this](const auto &lhs, const auto &rhs) { return isVertexBetter(lhs, rhs); }),
        ...
        declareParam<bool>("use_k_nearest", this, &BiAIT::setUseKNearest, &BiAIT::getUseKNearest, "0,1");
        declareParam<double>("rewire_factor", this, &BiAIT::setRewireFactor, &BiAIT::getRewireFactor, "1.0:0.01:3.0");
        declareParam<std::size_t>("batch_size", this, &BiAIT::setBatchSize, &BiAIT::getBatchSize, "1:1:1000");
...
//            implicitGraph_.setSampleDeterministic(batchSize_);
    ...
    base::PlannerStatus::StatusType
    BiAIT::ensureStartAndGoalStates(const base::PlannerTerminationCondition &terminationCondition) {
    ...
    base::PlannerStatus BiAIT::solve(const base::PlannerTerminationCondition &terminationCondition) {
        ...
        base::PlannerStatus::StatusType startGoalStatus = ensureStartAndGoalStates(terminationCondition);
        ...
        OMPL_INFORM("%s: Solving the given planning problem. \nThe current best solution cost is %.4f. ", name_.c_str(),
                    solutionCost_.value());
    ...
    std::vector<Edge> BiAIT::getOutgoingEdges(const std::shared_ptr<Vertex> &vertex) const {
        // assert vertex is a valid vertex;
        assert(vertex->getCategory()[4] || vertex->getCategory()[1]);

        // And find the outgoing edge category;
        std::bitset<5> outgoingValidEdgeCategory{};     std::bitset<5> outgoingLazyEdgeCategory{};
        std::function<std::array<base::Cost, 3u> (const base::OptimizationObjectivePtr &,
                                                  const std::shared_ptr<Vertex> &,
                                                  const std::shared_ptr<Vertex> &)> computeEdgeKey;

        // Insert the current valid children;
        std::vector<Edge> outgoingEdges{};
        if (vertex->getCategory()[4]) {
            outgoingValidEdgeCategory = 0b10000;        outgoingLazyEdgeCategory = 0b01000;
            computeEdgeKey = ompl::geometric::BiAIT::computeForwardEdgeKey;
            for (const auto &elem: vertex->getForwardValidChildren()) {
                outgoingEdges.emplace_back(vertex, elem, outgoingValidEdgeCategory, computeEdgeKey(optObjPtr_, vertex, elem));
            }
        } else if (vertex->getCategory()[1]) {
            outgoingValidEdgeCategory = 0b00010;        outgoingLazyEdgeCategory = 0b00001;
        ...
        for (const auto &elem: implicitGraph_.getNeighbors(vertex)) {
            ...
            if (elem->hasBlacklistedChild(vertex) || vertex->hasBlacklistedChild(elem)) continue;
            outgoingEdges.emplace_back(vertex, elem, outgoingLazyEdgeCategory, computeEdgeKey(optObjPtr_, vertex, elem));
        }
        ...
        if (vertex->hasForwardLazyChild()) {
            for (const auto &elem:  vertex->getForwardLazyChildren()) {
                outgoingEdges.emplace_back(vertex, elem, 0b01000, computeEdgeKey(optObjPtr_, vertex, elem));
            }
        }
...
//        }
    ...
    BiAIT::computeForwardEdgeKey(const base::OptimizationObjectivePtr &optObjPtr, const std::shared_ptr<Vertex> &parent,
...
//        std::cout << "[1]: " << parent->costToStart_.value() << "[2]: " << edgeCostHeuristic.value() << "[3]: " << child->heuristicCost_g_ReverseLazy_.value() << std::endl;
    ...
    std::array<base::Cost, 3u>
    BiAIT::computeReverseEdgeKey(const base::OptimizationObjectivePtr &optObjPtr, const std::shared_ptr<Vertex> &parent,
                                 const std::shared_ptr<Vertex> &child) {
        ...
        return {
                optObjPtr->combineCosts(
                        optObjPtr->combineCosts(parent->costToGoal_, edgeCostHeuristic), child->heuristicCost_g_ForwardLazy_),
    ...
    std::array<base::Cost, 2u> BiAIT::computeForwardVertexKey(const std::shared_ptr<Vertex> & vertex) const{
        // { min(h_hat{g-FL}(x), h_hat{rhs-FL}(x)) + g_hat{goal}, min(h_hat{g-FL}(x), h_hat{rhs-FL}(x))};
        auto betterHeuristic = optObjPtr_->betterCost(vertex->heuristicCost_g_ForwardLazy_,
                                                            vertex->heuristicCost_rhs_ForwardLazy_);
        return {
    ...
    std::array<base::Cost, 2u> BiAIT::computeReverseVertexKey(const std::shared_ptr<Vertex> & vertex) const{
        //  { min(h_hat{g-RL}(x), h_hat{rhs-RL}(x)) + g_hat{start}, min(h_hat{g-RL}(x), h_hat{rhs-RL}(x))};
        auto betterHeuristic = optObjPtr_->betterCost(vertex->heuristicCost_g_ReverseLazy_,
                                                            vertex->heuristicCost_rhs_ReverseLazy_);
        return {
                optObjPtr_->combineCosts(betterHeuristic,
                                         computeCostToStartHeuristic(vertex)),
    ...
    base::Cost BiAIT::computeMeetLazyKey(const std::shared_ptr<Vertex> & parent, const std::shared_ptr<Vertex> & child){
        // Make sure parent in FL tree and child in RL tree;
        return optObjPtr_->combineCosts(optObjPtr_->combineCosts(parent->heuristicCost_g_ForwardLazy_,
                                        optObjPtr_->motionCostHeuristic(parent->getState(), child->getState())),
                                        child->heuristicCost_g_ReverseLazy_);
    }


    base::Cost BiAIT::computeMeetValidKey(const Edge & edge, const base::Cost & edgeCost) const {
        // The meetEdge key is parent->costToCome + edgeCost + child->costToGo;
        return optObjPtr_->combineCosts(optObjPtr_->combineCosts(edge.getParent()->costToStart_,
                                                                 edgeCost),
    ...
    void BiAIT::insertOrUpdateForwardValidQueue(const Edge &edge) {
        // If the edge is an meetLazyEdge or the parent and child lies in different lazy trees;
        // Currently we omit this situation and leave it for the meetValidEdge to handle;
        if(findInMeetLazyQueue(edge) || edge.getChild()->getCategory()[0] || edge.getChild()->getCategory()[1]){
            return;
        }
        if((edge.getParent()->getCategory() | edge.getChild()->getCategory())[0] && (edge.getParent()->getCategory() | edge.getChild()->getCategory())[3]){
            return;
        }
        // If the edge is included in the reverseValidQueue, we do nothing and return;
        // TODO: May this be the chance for trees meet;
        const auto iter_ReverseQueue = std::find_if(edge.getParent()->validQueueIncomingLookupFromGoal_.begin(),
        ...
        if(iter_ReverseQueue != edge.getParent()->validQueueIncomingLookupFromGoal_.end())
            return;

        // Check if the edge is already in the queue and can be updated;
        const auto iter_ForwardQueue = std::find_if(edge.getChild()->validQueueIncomingLookupFromStart_.begin(),
                                       edge.getChild()->validQueueIncomingLookupFromStart_.end(),
                                       [&edge](const auto elem) {
                                           return elem->data.getParent()->getId() == edge.getParent()->getId();
                                       });
        if (iter_ForwardQueue != edge.getChild()->validQueueIncomingLookupFromStart_.end()) {
            // Use the incoming lookup of the child;
            // Assert it is already in the outgoing lookup in of the parent;
            assert(std::find_if(edge.getParent()->validQueueOutgoingLookupFromStart_.begin(),
            ...
            // This edge exists in the queue, if the new sortKey is better than the old, update;
...
//            }
    ...
    void BiAIT::insertOrUpdateReverseValidQueue(const Edge &edge) {
        // If the edge is an meetLazyEdge or the parent and child lies in different lazy trees;
        // Currently we omit this situation and leave it for the meetValidEdge to handle;
        if(findInMeetLazyQueue(edge) || edge.getChild()->getCategory()[3] || edge.getChild()->getCategory()[4]){
            return;
        }
        if((edge.getParent()->getCategory() | edge.getChild()->getCategory())[0] && (edge.getParent()->getCategory() | edge.getChild()->getCategory())[3]){
            return;
        ...
        // TODO: May this be the chance for trees meet;
        const auto iter_ForwardQueue = std::find_if(edge.getParent()->validQueueIncomingLookupFromStart_.begin(),
                                                    edge.getParent()->validQueueIncomingLookupFromStart_.end(),
                                                    [&edge](const auto elem) {
                                                        return elem->data.getParent()->getId() == edge.getChild()->getId();
                                                    });
        if(iter_ForwardQueue != edge.getParent()->validQueueIncomingLookupFromStart_.end())
            return;

        // Check if the edge is already in the queue and can be updated;
        const auto iter_ReverseQueue = std::find_if(edge.getChild()->validQueueIncomingLookupFromGoal_.begin(),
                                       edge.getChild()->validQueueIncomingLookupFromGoal_.end(),
                                       [&edge](const auto elem) {
                                           return elem->data.getParent()->getId() == edge.getParent()->getId();
                                       });
        if (iter_ReverseQueue != edge.getChild()->validQueueIncomingLookupFromGoal_.end()) {
            // Use the incoming lookup of the child;
            // Assert it is already in the outgoing lookup in of the parent;
            assert(std::find_if(edge.getParent()->validQueueOutgoingLookupFromGoal_.begin(),
...
//            }
    ...
    void BiAIT::insertOrUpdateForwardLazyQueue(const std::shared_ptr<Vertex> &vertex) {
        ...
        if(vertex->forwardLazyQueuePointer_ != nullptr){
            ...
            vertex->forwardLazyQueuePointer_->data.first = computeForwardVertexKey(vertex);
    ...
    void BiAIT::insertOrUpdateReverseLazyQueue(const std::shared_ptr<Vertex> &vertex) {
        // Get the pointer to the element in the queue;
        if(vertex->reverseLazyQueuePointer_){
            // Update it as it is already in the queue;
            vertex->reverseLazyQueuePointer_->data.first = computeReverseVertexKey(vertex);
...
//        DebugBiAIT::printDataInEdgeQueue(reverseValidQueue_, "RVQ: ");
        ...
        if(auto queueElemPointer = findInMeetLazyQueue(edge)){
            ...
            // Using the push_back instead of the emplace_back since the pointerToNewElem will be emplaced multiple times;
        ...
        if(performFL || performFV || performRL || performRV){
            ...
            if(implicitGraph_.addSamples(batchSize_, terminationCondition)){
                ...
                if(pis_.haveMoreStartStates() || pis_.haveMoreGoalStates()){
                    ...
                    implicitGraph_.updateStartAndGoalStates(base::plannerAlwaysTerminatingCondition(), &pis_);
                }
...
//                DebugBiAIT::modifyVertex(meetLazyEdgeQueue_, "AddSample ");
    ...
    void BiAIT::updateExactSolution() {
        ...
        if(optObjPtr_->isCostBetterThan(meetValidEdgeQueue_.top()->data.second.getMeetValidEdgeKey(), solutionCost_)){
            auto pathToGoal = getPathToGoal();
            if(pathToGoal != nullptr){
                base::PlannerSolution solution(pathToGoal);
                solutionCost_ = meetValidEdgeQueue_.top()->data.second.getMeetValidEdgeKey();
    ...
    void BiAIT::informAboutPlannerStatus(base::PlannerStatus::StatusType status) const {
        switch (status) {
            ...
            case base::PlannerStatus::StatusType::APPROXIMATE_SOLUTION: {
                OMPL_ERROR("%s (%u iterations): APPROXIMATE_SOLUTION is disabled. Solution not found.", name_.c_str(),
                           numIterations_);
                break;
            }
            case base::PlannerStatus::StatusType::TIMEOUT: {
                OMPL_INFORM("%s (%u iterations): Solution not found.", name_.c_str(), numIterations_);
                break;
            }
            ...
            case base::PlannerStatus::StatusType::TYPE_COUNT: {
                OMPL_INFORM(
                        "%s (%u iterations): Unable to solve the given planning problem. The PlannerStatus::StatusType is"
                        "UNKNOWN, INVALID_START, INVALID_GOAL, UNRECOGNIZED_GOAL_TYPE, CRASH, ABORT, or TYPE_COUNT",
                        name_.c_str(),
                        numIterations_);
            }
        }
    ...
    void BiAIT::informRunTimeStatus() const {
        OMPL_INFORM(
                "Run Time Info: \t[ %s ]\t Iteration: [ %zu ]; \t BatchId: [ %zu ]; \t Sampled states [ %u ]; \t Valid samples: [ %zu ] (%.1f); \t MeetLazyEdgeQueue.size(): [ %zu ]\n"
    ...
    void BiAIT::informNewSolution() const {
        OMPL_INFORM("%s (%u iterations): Found a new exact solution of cost %.4f. Sampled a total of %u states, %u "
                    ...
                    "(%.1f \%). "
                    "The [forward valid/forward lazy/reverse valid/reverse lazy] portion contains [%u/%u/%u/%u] vertices. "
                    "The [meet queue] have [%u] edges. The [start/goal] set have [%u/%u] vertices.",
                    name_.c_str(), numIterations_, solutionCost_.value(), implicitGraph_.getNumberOfSampledStates(),
                    implicitGraph_.getNumberOfValidSamples(),
                    implicitGraph_.getNumberOfSampledStates() == 0u ?
                    0.0 :
                    100.0 * (static_cast<double>(implicitGraph_.getNumberOfValidSamples()) /
    ...
    std::size_t BiAIT::countNumVerticesInReverseLazyPortion() const {
        ...
        auto vertices = implicitGraph_.getVertices();
        for (const auto &elem: vertices) {
//            assert(!implicitGraph_.isGoal(elem) && elem->getCategory().count() == 1);
    ...
    PathGeometricPtr BiAIT::getPathToGoal() const {
        ...
        if(bestMeetEdge->data.second.getParent()->getForwardValidParent() == nullptr || bestMeetEdge->data.second.getChild()->getReverseValidParent() == nullptr){
            ...
            Utility::removeFromVectorByValue(bestMeetEdge->data.second.getChild()->meetValidEdgeQueuePointer_, meetValidEdgeQueue_.top());
    ...
    bool BiAIT::isEdgeBetter(const Edge &lhs, const Edge &rhs) const {
        return std::lexicographical_compare(
                lhs.getEdgeKey().cbegin(), lhs.getEdgeKey().cend(), rhs.getEdgeKey().cbegin(), rhs.getEdgeKey().cend(),
    ...
    bool BiAIT::isVertexBetter(const Queuetypes::KeyVertexPair &lhs, const Queuetypes::KeyVertexPair &rhs) const {
        ...
        if (optObjPtr_->isCostEquivalentTo(lhs.first[0u], rhs.first[0u]) &&
            ...
            return std::lexicographical_compare(
                    ...
                    [this](const auto &a, const auto &b) { return optObjPtr_->isCostBetterThan(a, b); });
        }

    }


    base::Cost BiAIT::computeBestCostHeuristic(const std::shared_ptr<Vertex> &fromVertex,
        ...
        for (const auto &elem: toVectorOfVertex) {
            outputCost = optObjPtr_->betterCost(outputCost,
                                                optObjPtr_->motionCostHeuristic(
                                                                       fromVertex->getState(), elem->getState()));
        }
        return outputCost;
    }


    base::Cost BiAIT::computeBestCostHeuristic(const std::vector<std::shared_ptr<Vertex>> &fromVectorOfVertex,
                                               const std::shared_ptr<Vertex> &toVertex) const {
        base::Cost outputCost = optObjPtr_->infiniteCost();
        for (const auto &elem: fromVectorOfVertex) {
            outputCost = optObjPtr_->betterCost(outputCost,
                                                optObjPtr_->motionCostHeuristic(
                                                                       elem->getState(), toVertex->getState()));
        }
        return outputCost;
    }


    void BiAIT::updateCostToStartOfForwardValidDescendant(const std::shared_ptr<Vertex> &vertex) const {
        for (const auto &elem: vertex->getForwardValidChildren()) {
            elem->costToStart_ = optObjPtr_->combineCosts(vertex->costToStart_, elem->edgeCostFromStart_);
    ...
    void BiAIT::updateCostToGoalOfReverseValidDescendant(const std::shared_ptr<Vertex> &vertex) const {
        for (const auto &elem: vertex->getReverseValidChildren()) {
            elem->costToGoal_ = optObjPtr_->combineCosts(vertex->costToGoal_, elem->edgeCostFromGoal_);
            if (elem->getCategory()[2]) {
                insertOrUpdateMeetValidEdge(elem);
            }
    ...
    void BiAIT::updateCostToStartOfRVPredecessor(const std::shared_ptr<Vertex> & vertex) const {
        if(vertex->hasReverseValidParent()){
            auto RVParent = vertex->getReverseValidParent();
            if(optObjPtr_->isCostBetterThan(optObjPtr_->combineCosts(vertex->costToStart_, vertex->edgeCostFromGoal_),
                                            RVParent->costToStart_)){
                RVParent->costToStart_ = optObjPtr_->combineCosts(vertex->costToStart_, vertex->edgeCostFromGoal_);
...
//    }
    ...
    void BiAIT::insertOrUpdateMeetValidEdge(Edge & selectedValidEdge, const base::Cost & edgeCost) {
        // Forcing the front-end pass the edgeCost, because calculating the edgeCost is costly;
        selectedValidEdge.setMeetValidEdgeKey(computeMeetValidKey(selectedValidEdge, edgeCost));
        if(auto queuePointer = findInMeetValidQueue(selectedValidEdge)){
            // Update meet valid queue;
            if(optObjPtr_->isCostBetterThan(selectedValidEdge.getMeetValidEdgeKey(), queuePointer->data.second.getMeetValidEdgeKey())){
            ...
            // Using the push_back instead of the emplace_back since the pointerToNewElem will be emplaced multiple times;
            auto pointerToNewElem = meetValidEdgeQueue_.insert(std::make_pair(edgeCost, selectedValidEdge));
            pointerToNewElem->data.first = edgeCost;
            pointerToNewElem->data.second.getParent()->meetValidEdgeQueuePointer_.push_back(pointerToNewElem);
            pointerToNewElem->data.second.getChild()->meetValidEdgeQueuePointer_.push_back(pointerToNewElem);
    ...
    void BiAIT::insertOrUpdateMeetValidEdge(const std::shared_ptr<Vertex> &vertex) const {
        // Find the `vertex` related meetValidEdge(s);
        for(const auto & elem: vertex->meetValidEdgeQueuePointer_){
            if(elem == nullptr) continue;
            assert(vertex->getId() == elem->data.second.getParent()->getId() || vertex->getId() == elem->data.second.getChild()->getId());
            base::Cost newMeetValidKey = computeMeetValidKey(elem->data.second, elem->data.first);
            if(optObjPtr_->isCostBetterThan(newMeetValidKey, elem->data.second.getMeetValidEdgeKey())){
                // Update the related meetValidEdge(s);
                elem->data.second.setMeetValidEdgeKey(newMeetValidKey);
                meetValidEdgeQueue_.update(elem);
                // Propagate the updated information to start and goal precedents, respectively;
                elem->data.second.getParent()->costToGoal_ = optObjPtr_->combineCosts(elem->data.second.getChild()->costToGoal_, elem->data.first);
                elem->data.second.getChild()->costToStart_ = optObjPtr_->combineCosts(elem->data.second.getParent()->costToStart_, elem->data.first);
    ...
    void BiAIT::insertOrUpdateMeetValidEdge(const std::shared_ptr<Vertex> & parent, const std::shared_ptr<Vertex> & child){
        if(parent->hasWhitelistedChild(child) || motionValidatorPtr_->checkMotion(parent->getState(), child->getState())) {
            // Collision free!
            ...
            Edge meetValidEdge(parent, child, 0b00100,
                               {{optObjPtr_->infiniteCost(), optObjPtr_->infiniteCost(), optObjPtr_->infiniteCost()}});
            const auto edgeCost = optObjPtr_->motionCost(parent->getState(), child->getState());
            insertOrUpdateMeetValidEdge(meetValidEdge, edgeCost);
        }
    }


    void BiAIT::updateCostToStartOfReverseValidPrecedent(const std::shared_ptr<Vertex> & vertex) const{
        if(vertex->hasReverseValidParent()){
            if(optObjPtr_->isCostBetterThan(optObjPtr_->combineCosts(vertex->costToStart_, vertex->edgeCostFromGoal_),
                                            vertex->getReverseValidParent()->costToStart_)) {
                vertex->getReverseValidParent()->costToStart_ = optObjPtr_->combineCosts(vertex->costToStart_,
    ...
    void BiAIT::updateCostToGoalOfForwardValidPrecedent(const std::shared_ptr<Vertex> &vertex) const {
        if(vertex->hasForwardValidParent()){
            if(optObjPtr_->isCostBetterThan(optObjPtr_->combineCosts(vertex->costToGoal_, vertex->edgeCostFromStart_),
                                            vertex->getForwardValidParent()->costToGoal_)){
                vertex->getForwardValidParent()->costToGoal_ = optObjPtr_->combineCosts(vertex->costToGoal_,
...
//    }
    ...
    bool BiAIT::performReverseLazySearch() {
        if(reverseLazyQueue_.empty() || forwardValidQueue_.empty() || reverseValidQueue_.empty()) return false;
        const auto & bestEdge = optObjPtr_->isCostBetterThan(forwardValidQueue_.top()->data.getEdgeKey()[0u],
                                                             reverseValidQueue_.top()->data.getEdgeKey()[0u]) ?
                                forwardValidQueue_.top()->data: reverseValidQueue_.top()->data;
...
//                           forwardValidQueue_.top()->data.getEdgeKey()[0u]: reverseValidQueue_.top()->data.getEdgeKey()[0u];
    ...
    void BiAIT::iterateForwardLazySearch() {
        ...
        // If this vertex is also contained in the reverseValidQueue_, eliminate this vertex in RVQueue;
...
//        assertInconsistentInForwardLazy(vertex);
        ...
        if(optObjPtr_->isCostBetterThan(vertex->heuristicCost_rhs_ForwardLazy_, vertex->heuristicCost_g_ForwardLazy_) ||
           optObjPtr_->isCostEquivalentTo(vertex->heuristicCost_rhs_ForwardLazy_, vertex->heuristicCost_g_ForwardLazy_)) {
            ...
            vertex->heuristicCost_g_ForwardLazy_ = vertex->heuristicCost_rhs_ForwardLazy_;
    ...
    void BiAIT::iterateReverseLazySearch() {
        ...
        // If this vertex is also contained in the forwardValidQueue_, eliminate this vertex in FVQueue;
...
//        assertInconsistentInReverseLazy(vertex);
        ...
        if(optObjPtr_->isCostBetterThan(vertex->heuristicCost_rhs_ReverseLazy_, vertex->heuristicCost_g_ReverseLazy_) ||
           optObjPtr_->isCostEquivalentTo(vertex->heuristicCost_rhs_ReverseLazy_, vertex->heuristicCost_g_ReverseLazy_)) {
            // Make it consistent and update the vertex;
            vertex->heuristicCost_g_ReverseLazy_ = vertex->heuristicCost_rhs_ReverseLazy_;
            // Update the number of inconsistent targets in the reverse valid queue;
            numInconsistentOrUnconnectedTargetsInReverse_ -= vertex->validQueueIncomingLookupFromStart_.size();
        } else {
            // Make the vertex over-consistent;
            vertex->heuristicCost_g_ReverseLazy_ = optObjPtr_->infiniteCost();
            // Update the vertex and its neighbors;
    ...
    void BiAIT::iterateForwardValidSearch() {
        ...
        if(!(child->isConsistentInReverseLazySearch() && !implicitGraph_.isGoal(parent)  && !child->getCategory()[1] && !child->getCategory()[0])){
            return;
        }
        ++numProcessedEdges_;

        // If this edge is already in the forward valid tree, it is a free-by:
        if(child->hasForwardValidParent() && child->getForwardValidParent()->getId() == parent->getId()){
            insertOrUpdateForwardValidQueue(getOutgoingEdges(child));
            return;
        } else if (optObjPtr_->isCostBetterThan(optObjPtr_->combineCosts(
                  parent->costToStart_, optObjPtr_->motionCostHeuristic(parent->getState(), child->getState()))
                , child->costToStart_)) {
            ...
            if(parent->hasWhitelistedChild(child) || motionValidatorPtr_->checkMotion(parent->getState(), child->getState())) {
                ...
                if(optObjPtr_->isCostBetterThan(optObjPtr_->combineCosts(parent->costToStart_, edgeCost), child->costToStart_)){
...
//                        DebugBiAIT::modifyVertex(child->meetLazyEdgeQueuePointer_, "FV1 ");
                    ...
                    for(const auto & elem: implicitGraph_.getNeighbors(child)){
                        ...
                        if(child->hasBlacklistedChild(elem) || elem->hasBlacklistedChild(child)) continue;
                ...
                if(child->hasForwardLazyParent() && child->getForwardLazyParent()->getId() == parent->getId()) {
                    // Forward lazy branch;
    ...
    void BiAIT::iterateReverseValidSearch() {
        ...
        if(!(child->isConsistentInForwardLazySearch() && !implicitGraph_.isStart(parent) && !child->getCategory()[4] && !child->getCategory()[3])){
            return;
        }
        ++numProcessedEdges_;

        // If this edge is already in the reverse valid tree, it is a free-by:
        if(child->hasReverseValidParent() && child->getReverseValidParent()->getId() == parent->getId()){
            insertOrUpdateReverseValidQueue(getOutgoingEdges(child));
            return;
        } else if (optObjPtr_->isCostBetterThan(optObjPtr_->combineCosts(
                  parent->costToGoal_, optObjPtr_->motionCostHeuristic(parent->getState(), child->getState()))
                , child->costToGoal_)) {
            // Check if the edge can possibly improve the current solution;
            // If the edge can, then perform collision checking here;
            if(parent->hasWhitelistedChild(child) || motionValidatorPtr_->checkMotion(parent->getState(), child->getState())) {
                ...
                if(optObjPtr_->isCostBetterThan(optObjPtr_->combineCosts(parent->costToGoal_, edgeCost), child->costToGoal_)){
...
//                        DebugBiAIT::modifyVertex(child->meetLazyEdgeQueuePointer_, "RV1 ");
                ...
                child->blacklistAsChild(parent);
                // Repair both the forward lazy search and the reverse lazy search;
                if(child->hasReverseLazyParent() && child->getReverseLazyParent()->getId() == parent->getId()) {
                    // Reverse lazy branch;
...
//            if(!child->isConsistentInForwardLazySearch() || (propagateFrom != nullptr && child->getId() == propagateFrom->getId())) continue;
        ...
        if(!optObjPtr_->isFinite(vertex->heuristicCost_g_ForwardLazy_) && vertex->hasReverseLazyParent()){
            invalidateRLToGoalWhenInvalidatingRL(vertex->getReverseLazyParent(), vertex);
        }
    }


    void BiAIT::invalidateFLToStartWhenInvalidatingFL(const std::shared_ptr<Vertex> &vertex,
...
//            if(!child->isConsistentInReverseLazySearch() || (propagateFrom != nullptr && child->getId() == propagateFrom->getId())) continue;
        ...
        if(!optObjPtr_->isFinite(vertex->heuristicCost_g_ReverseLazy_) && vertex->hasForwardLazyParent()){
            invalidateFLToStartWhenInvalidatingFL(vertex->getForwardLazyParent(), vertex);
        }
    }
...
//        if(vertex->getCategory()[1]) return;
            ...
            for(const auto & meetEdge : vertex->meetLazyEdgeQueuePointer_) {
                if(meetEdge == nullptr || !meetEdge->data.getParent().lock() || !meetEdge->data.getChild().lock()) {
                    if(meetEdge != nullptr && meetEdge->data.getParent().lock()){
                        Utility::removeFromVectorByValue(meetEdge->data.getParent().lock()->meetLazyEdgeQueuePointer_, meetEdge);
                    }
                    meetLazyEdgeQueue_.remove(meetEdge);
                    continue;
                }
                ...
                if(optObjPtr_->isCostEquivalentTo(
                        ...
                        optObjPtr_->combineCosts(meetEdge->data.getChild().lock()->heuristicCost_g_ReverseLazy_,
                                                 optObjPtr_->motionCostHeuristic(meetEdge->data.getChild().lock()->getState(),
                                                                                 meetEdge->data.getParent().lock()->getState())))) {
                    invalidateFLWhenInvalidatingRL(meetEdge->data.getParent().lock(), nullptr);
                }
                meetLazyEdgeQueue_.remove(meetEdge);
            }
            vertex->meetLazyEdgeQueuePointer_.clear();
        ...
        // Reset the cost to come from the goal and the reverse lazy children unless the vertex is itself a start;
...
//        updateVertexInRLSearch(vertex);
    ...
    void BiAIT::invalidateFLWhenInvalidatingRL(const std::shared_ptr<Vertex> &vertex,
                                               const std::shared_ptr<Vertex> &propagateFrom) {
        base::Cost bestHeuristic = optObjPtr_->infiniteCost();
        for(const auto & child: vertex->getForwardLazyChildren()){
//            if(!child->isConsistentInReverseLazySearch() || (propagateFrom != nullptr && child->getId() == propagateFrom->getId())) continue;
    ...
    void BiAIT::invalidateRLToGoalWhenInvalidatingRL(const std::shared_ptr<Vertex> &vertex,
                                                     const std::shared_ptr<Vertex> &propagateFrom) {
...
//            }
    ...
    void BiAIT::updateVertexInFLSearch(const std::shared_ptr<Vertex> &vertex) {
        ...
        if(implicitGraph_.isStart(vertex) || !vertex->validQueueIncomingLookupFromGoal_.empty() ) {
            return;
        } else if(!vertex->getCategory()[0] && !vertex->getCategory()[1]){
            // We process this vertex and try to connect or update it to the current associated tree;
...
//            std::cout << "bestParent: " << bestParent->getCategory() << "\tisLeafNode: " << !vertex->hasReverseLazyChild() << std::endl;
            ...
            // The vertex is orphaned, currently; reset the forward lazy parent if there is one;
            if(vertex->hasForwardLazyParent()) {
                vertex->getForwardLazyParent()->removeFromForwardLazyChildren(vertex->getId());
                vertex->resetForwardLazyParent();
            }
        ...
        if(!vertex->isConsistentInForwardLazySearch()) {
            ...
            if(auto elemPtr = vertex->forwardLazyQueuePointer_) {
                // Remove this vertex from the open queue if it is in the queue and is consistent;
    ...
    void BiAIT::updateVertexInFLSearchToMeetQueue(const std::shared_ptr<Vertex> &vertex) {
        // `vertex` is in the RL tree, we find the best parent in FL tree for RL, if both vertex parent is consistent in RL
        // and parent is consistent in FL, we connect vertex and the found parent with `meetLazyEdge`;
        // Get the best parent for this vertex;
        if(!vertex->isConsistentInReverseLazySearch() || !optObjPtr_->isFinite(vertex->heuristicCost_g_ReverseLazy_))
            return;
        std::weak_ptr<Vertex> bestParent;
        base::Cost bestCost = optObjPtr_->infiniteCost();

        bool foundBestParent = updateVertexInFLSearch_bestWeakParent(vertex, bestParent, bestCost);
        if(!foundBestParent || !bestParent.lock()->getCategory()[3] || !bestParent.lock()->isConsistentInForwardLazySearch())
            return;
        ...
        auto meetEdgeCost = optObjPtr_->motionCostHeuristic(bestParent.lock()->getState(), vertex->getState());
        if(optObjPtr_->isCostBetterThan(optObjPtr_->combineCosts(vertex->heuristicCost_g_ReverseLazy_, meetEdgeCost),
                                        bestParent.lock()->heuristicCost_rhs_ReverseLazy_)){
            bestParent.lock()->heuristicCost_rhs_ReverseLazy_ = optObjPtr_->combineCosts(vertex->heuristicCost_g_ReverseLazy_,
                                                                                  meetEdgeCost);
            bestParent.lock()->heuristicCost_g_ReverseLazy_ = bestParent.lock()->heuristicCost_rhs_ReverseLazy_;
        ...
        if(optObjPtr_->isCostBetterThan(optObjPtr_->combineCosts(bestParent.lock()->heuristicCost_g_ForwardLazy_, meetEdgeCost),
                                        vertex->heuristicCost_g_ForwardLazy_)){
            vertex->heuristicCost_rhs_ForwardLazy_ = optObjPtr_->combineCosts(bestParent.lock()->heuristicCost_g_ForwardLazy_,
                                                                            meetEdgeCost);
            vertex->heuristicCost_g_ForwardLazy_ = vertex->heuristicCost_rhs_ForwardLazy_;
            if(vertex->forwardLazyQueuePointer_){
                forwardLazyQueue_.remove(vertex->forwardLazyQueuePointer_);
    ...
    void BiAIT::updateVertexInFLSearch_bestParent(const std::shared_ptr<Vertex> &vertex,
                                                  std::shared_ptr<Vertex> &bestParent, base::Cost &bestCost) {
        // Check all neighbors in RGG, and record the best lazy parent and the cost to root through this parent;
        for(const auto & elem: implicitGraph_.getNeighbors(vertex)) {
            if(elem->getId() != vertex->getId() && !elem->hasBlacklistedChild(vertex) && !vertex->hasBlacklistedChild(
                    elem)) {
                auto edgeCost = optObjPtr_->motionCostHeuristic(elem->getState(), vertex->getState());
                auto iterateCost = optObjPtr_->combineCosts(elem->heuristicCost_g_ForwardLazy_, edgeCost);
        ...
        for(const auto & reverseValidChild: vertex->getReverseValidChildren()) {
            auto edgeCost = optObjPtr_->motionCostHeuristic(reverseValidChild->getState(), vertex->getState());
            auto iterateCost = optObjPtr_->combineCosts(reverseValidChild->heuristicCost_g_ForwardLazy_, edgeCost);
            if(optObjPtr_->isCostBetterThan(iterateCost, bestCost)) {
        ...
        for(const auto & forwardValidChild : vertex->getForwardValidChildren()){
            auto edgeCost = optObjPtr_->motionCostHeuristic(forwardValidChild->getState(), vertex->getState());
            auto iterateCost = optObjPtr_->combineCosts(forwardValidChild->heuristicCost_g_ForwardLazy_, edgeCost);
            if(optObjPtr_->isCostBetterThan(iterateCost, bestCost)) {
        ...
        if(vertex->hasForwardValidParent()) {
            ...
            auto edgeCost = optObjPtr_->motionCostHeuristic(forwardValidParent->getState(), vertex->getState());
            auto candidateCost = optObjPtr_->combineCosts(forwardValidParent->heuristicCost_g_ForwardLazy_, edgeCost);
        ...
        if(vertex->hasReverseValidParent()) {
            ...
            auto edgeCost = optObjPtr_->motionCostHeuristic(reverseValidParent->getState(), vertex->getState());
            auto candidateCost = optObjPtr_->combineCosts(reverseValidParent->heuristicCost_g_ForwardLazy_, edgeCost);
            if(optObjPtr_->isCostBetterThan(candidateCost, bestCost)) {
    ...
    bool BiAIT::updateVertexInFLSearch_bestWeakParent(const std::shared_ptr<Vertex> &vertex,
                                                  std::weak_ptr<Vertex> &bestParent, base::Cost &bestCost) {
        ...
        for(const auto & reverseValidChild: vertex->getReverseValidWeakChildren()) {
            auto edgeCost = optObjPtr_->motionCostHeuristic(reverseValidChild.lock()->getState(), vertex->getState());
            auto iterateCost = optObjPtr_->combineCosts(reverseValidChild.lock()->heuristicCost_g_ForwardLazy_, edgeCost);
            if(optObjPtr_->isCostBetterThan(iterateCost, bestCost)) {
                bestParent = reverseValidChild;
                bestCost = iterateCost;
                foundBestParent = true;
        ...
        for(const auto & forwardValidChild : vertex->getForwardValidWeakChildren()){
            auto edgeCost = optObjPtr_->motionCostHeuristic(forwardValidChild.lock()->getState(), vertex->getState());
            auto iterateCost = optObjPtr_->combineCosts(forwardValidChild.lock()->heuristicCost_g_ForwardLazy_, edgeCost);
            if(optObjPtr_->isCostBetterThan(iterateCost, bestCost)) {
                bestParent = forwardValidChild;
                bestCost = iterateCost;
                foundBestParent = true;
        ...
        if(vertex->hasForwardValidParent()) {
            ...
            auto edgeCost = optObjPtr_->motionCostHeuristic(forwardValidParent.lock()->getState(), vertex->getState());
            auto candidateCost = optObjPtr_->combineCosts(forwardValidParent.lock()->heuristicCost_g_ForwardLazy_, edgeCost);
            if(optObjPtr_->isCostBetterThan(candidateCost, bestCost)) {
                bestParent = forwardValidParent;
                bestCost = candidateCost;
                foundBestParent = true;
        ...
        if(vertex->hasReverseValidParent()) {
            ...
            auto edgeCost = optObjPtr_->motionCostHeuristic(reverseValidParent.lock()->getState(), vertex->getState());
            auto candidateCost = optObjPtr_->combineCosts(reverseValidParent.lock()->heuristicCost_g_ForwardLazy_, edgeCost);
            if(optObjPtr_->isCostBetterThan(candidateCost, bestCost)) {
                bestParent = reverseValidParent;
                bestCost = candidateCost;
                foundBestParent = true;
    ...
    void BiAIT::updateVertexInRLSearch(const std::shared_ptr<Vertex> &vertex) {
        ...
        if(implicitGraph_.isGoal(vertex) || !vertex->validQueueIncomingLookupFromStart_.empty()) {
            return;
    ...
    void BiAIT::updateVertexInRLSearchToRLTree(const std::shared_ptr<Vertex> &vertex) {
        // Get the best parent for this vertex;
...
//                std::cout << DebugBiAIT::traceGoal(implicitGraph_, vertex) << std::endl;
    ...
    void BiAIT::updateVertexInRLSearchToMeetQueue(const std::shared_ptr<Vertex> &vertex) {
        // `vertex` is in the FL tree, we find the best parent in RL tree for FL, if both vertex is consistent in FL
        // and parent is consistent in RL, we connect vertex and the found parent with `meetLazyEdge`;
        // Get the best parent for this vertex;
        if(!vertex->isConsistentInForwardLazySearch() || !optObjPtr_->isFinite(vertex->heuristicCost_g_ForwardLazy_))
            return;
        std::weak_ptr<Vertex> bestParent;
        base::Cost bestCost = optObjPtr_->infiniteCost();

        bool foundBestParent = updateVertexInRLSearch_bestWeakParent(vertex, bestParent, bestCost);
        if(!foundBestParent || !bestParent.lock()->getCategory()[0] || !bestParent.lock()->isConsistentInReverseLazySearch())
            return;
        // lazyTreesMeet, `vertex`: FL, `bestParent`: RL;
        insertOrUpdateMeetLazyQueue(vertex, bestParent);

        // Propagate the g-value and rhs-value in both lazy trees;
        auto meetEdgeCost = optObjPtr_->motionCostHeuristic(vertex->getState(), bestParent.lock()->getState());
        if(optObjPtr_->isCostBetterThan(optObjPtr_->combineCosts(vertex->heuristicCost_g_ForwardLazy_, meetEdgeCost),
                                        bestParent.lock()->heuristicCost_g_ForwardLazy_)){
            bestParent.lock()->heuristicCost_rhs_ForwardLazy_ = optObjPtr_->combineCosts(vertex->heuristicCost_g_ForwardLazy_,
                                                                                  meetEdgeCost);
            bestParent.lock()->heuristicCost_g_ForwardLazy_ = bestParent.lock()->heuristicCost_rhs_ForwardLazy_;
        ...
        if(optObjPtr_->isCostBetterThan(optObjPtr_->combineCosts(bestParent.lock()->heuristicCost_g_ReverseLazy_, meetEdgeCost),
                                        vertex->heuristicCost_g_ReverseLazy_)){
            vertex->heuristicCost_rhs_ReverseLazy_ = optObjPtr_->combineCosts(bestParent.lock()->heuristicCost_g_ReverseLazy_,
                                                                              meetEdgeCost);
            vertex->heuristicCost_g_ReverseLazy_ = vertex->heuristicCost_rhs_ReverseLazy_;
            if(vertex->reverseLazyQueuePointer_){
                reverseLazyQueue_.remove(vertex->reverseLazyQueuePointer_);
    ...
    void BiAIT::updateVertexInRLSearch_bestParent(const std::shared_ptr<Vertex> &vertex,
                                                  std::shared_ptr<Vertex> &bestParent, base::Cost &bestCost) {
        // Check all neighbors in RGG, and record the best lazy parent and the cost to root through this parent;
        for(const auto & elem: implicitGraph_.getNeighbors(vertex)) {
            if(elem->getId() != vertex->getId() && !elem->hasBlacklistedChild(vertex) && !vertex->hasBlacklistedChild(
                    elem)) {
                auto edgeCost = optObjPtr_->motionCostHeuristic(elem->getState(), vertex->getState());
                auto iterateCost = optObjPtr_->combineCosts(elem->heuristicCost_g_ReverseLazy_, edgeCost);
                if(optObjPtr_->isCostBetterThan(iterateCost, bestCost)){
                    bestParent = elem;
                    bestCost = iterateCost;
                }
        ...
        for(const auto & forwardValidChild: vertex->getForwardValidChildren()) {
            auto edgeCost = optObjPtr_->motionCostHeuristic(forwardValidChild->getState(), vertex->getState());
            auto iterateCost = optObjPtr_->combineCosts(forwardValidChild->heuristicCost_g_ReverseLazy_, edgeCost);
            if(optObjPtr_->isCostBetterThan(iterateCost, bestCost)) {
                bestParent = forwardValidChild;
                bestCost = iterateCost;
            }
        ...
        for(const auto & reverseValidChild: vertex->getReverseValidChildren()) {
            auto edgeCost = optObjPtr_->motionCostHeuristic(reverseValidChild->getState(), vertex->getState());
            auto iterateCost = optObjPtr_->combineCosts(reverseValidChild->heuristicCost_g_ReverseLazy_, edgeCost);
            if(optObjPtr_->isCostBetterThan(iterateCost, bestCost)) {
                bestParent = reverseValidChild;
                bestCost = iterateCost;
            }
        ...
        if(vertex->hasForwardValidParent()) {
            auto forwardValidParent = vertex->getForwardValidParent();
            auto edgeCost = optObjPtr_->motionCostHeuristic(forwardValidParent->getState(), vertex->getState());
            auto candidateCost = optObjPtr_->combineCosts(forwardValidParent->heuristicCost_g_ReverseLazy_, edgeCost);
            if(optObjPtr_->isCostBetterThan(candidateCost, bestCost)) {
                bestParent = forwardValidParent;
                bestCost = candidateCost;
            }
        ...
        if(vertex->hasReverseValidParent()) {
            auto reverseValidParent = vertex->getReverseValidParent();
            auto edgeCost = optObjPtr_->motionCostHeuristic(reverseValidParent->getState(), vertex->getState());
            auto candidateCost = optObjPtr_->combineCosts(reverseValidParent->heuristicCost_g_ReverseLazy_, edgeCost);
            if(optObjPtr_->isCostBetterThan(candidateCost, bestCost)) {
                bestParent = reverseValidParent;
                bestCost = candidateCost;
            }
    ...
    bool BiAIT::updateVertexInRLSearch_bestWeakParent(const std::shared_ptr<Vertex> &vertex,
                                                  std::weak_ptr<Vertex> &bestParent, base::Cost &bestCost) {
        bool foundBestParent{false};
        // Check all neighbors in RGG, and record the best lazy parent and the cost to root through this parent;
        for(const auto & elem: implicitGraph_.getNeighbors(vertex)) {
        ...
        for(const auto & forwardValidChild: vertex->getForwardValidWeakChildren()) {
            auto edgeCost = optObjPtr_->motionCostHeuristic(forwardValidChild.lock()->getState(), vertex->getState());
            auto iterateCost = optObjPtr_->combineCosts(forwardValidChild.lock()->heuristicCost_g_ReverseLazy_, edgeCost);
            if(optObjPtr_->isCostBetterThan(iterateCost, bestCost)) {
                bestParent = forwardValidChild;
                bestCost = iterateCost;
                foundBestParent = true;
        ...
        for(const auto & reverseValidChild: vertex->getReverseValidWeakChildren()) {
            auto edgeCost = optObjPtr_->motionCostHeuristic(reverseValidChild.lock()->getState(), vertex->getState());
            auto iterateCost = optObjPtr_->combineCosts(reverseValidChild.lock()->heuristicCost_g_ReverseLazy_, edgeCost);
            if(optObjPtr_->isCostBetterThan(iterateCost, bestCost)) {
                bestParent = reverseValidChild;
                bestCost = iterateCost;
                foundBestParent = true;
        ...
        if(vertex->hasForwardValidParent()) {
            auto forwardValidParent = vertex->getForwardValidWeakParent();
            auto edgeCost = optObjPtr_->motionCostHeuristic(forwardValidParent.lock()->getState(), vertex->getState());
            auto candidateCost = optObjPtr_->combineCosts(forwardValidParent.lock()->heuristicCost_g_ReverseLazy_, edgeCost);
            if(optObjPtr_->isCostBetterThan(candidateCost, bestCost)) {
                bestParent = forwardValidParent;
                bestCost = candidateCost;
                foundBestParent = true;
        ...
        if(vertex->hasReverseValidParent()) {
            auto reverseValidParent = vertex->getReverseValidWeakParent();
            auto edgeCost = optObjPtr_->motionCostHeuristic(reverseValidParent.lock()->getState(), vertex->getState());
            auto candidateCost = optObjPtr_->combineCosts(reverseValidParent.lock()->heuristicCost_g_ReverseLazy_, edgeCost);
            if(optObjPtr_->isCostBetterThan(candidateCost, bestCost)) {
                bestParent = reverseValidParent;
                bestCost = candidateCost;
                foundBestParent = true;
    ...
    void BiAIT::updateForwardLazyNeighbors(const std::shared_ptr<Vertex> &vertex) {
        // Start with forward lazy children, because if this vertex becomes the parent of a neighbor, that neighbor would
        // be updated again as part of the forward lazy children;
        for(const auto & elem: vertex->getForwardLazyChildren()) {
            updateVertexInFLSearch(elem);
        }
        // AIT: we can now process the neighbors;
        for(const auto & elem : implicitGraph_.getNeighbors(vertex)){
            if(elem->getId() != vertex->getId() && !elem->hasBlacklistedChild(vertex) && !vertex->hasBlacklistedChild(elem)) {
                updateVertexInFLSearch(elem);
            }
        }
        // AIT: we also need to process the forward search children; (BiAIT: we also need to process the reverse valid search child)
        for(const auto &elem : vertex->getReverseValidChildren()){
            updateVertexInFLSearch(elem);
        }
        // AIT: we also need to update the forward search parent if it exists; (BiAIT: update the valid parent if it exists)
        if(vertex->hasForwardValidParent()) {
    ...
    void BiAIT::updateReverseLazyNeighbors(const std::shared_ptr<Vertex> &vertex) {
        // Start with reverse lazy children, because if this vertex becomes the parent of a neighbor, that neighbor would
        // be updated again as part of the reverse lazy children;
        // AIT: we can now process the neighbors;
        for(const auto & elem : vertex->getReverseLazyChildren()) {
    ...
    void BiAIT::propagateCostFromStartInRLTree(const std::shared_ptr<Vertex> & vertex){
        if(implicitGraph_.isGoal(vertex)) return;
        if(const auto parent = vertex->getReverseLazyParent()){
            auto candidateParentCostFromStart = optObjPtr_->combineCosts(
                    vertex->heuristicCost_g_ForwardLazy_,
                    optObjPtr_->motionCostHeuristic(vertex->getState(), parent->getState()));
            if(optObjPtr_->isCostBetterThan(candidateParentCostFromStart, parent->heuristicCost_g_ForwardLazy_)){
                parent->heuristicCost_rhs_ForwardLazy_ = candidateParentCostFromStart;
                parent->heuristicCost_g_ForwardLazy_ = parent->heuristicCost_rhs_ForwardLazy_;
    ...
    void BiAIT::propagateCostToGoalInFLTree(const std::shared_ptr<Vertex> & vertex){
        ...
        if(const auto parent = vertex->getForwardLazyParent()){
            auto candidateParentCostToGoal = optObjPtr_->combineCosts(
                    vertex->heuristicCost_g_ReverseLazy_,
                    optObjPtr_->motionCostHeuristic(parent->getState(), vertex->getState()));
            if(optObjPtr_->isCostBetterThan(candidateParentCostToGoal, parent->heuristicCost_g_ReverseLazy_)){
                parent->heuristicCost_rhs_ReverseLazy_ = candidateParentCostToGoal;
                parent->heuristicCost_g_ReverseLazy_ = parent->heuristicCost_rhs_ReverseLazy_;
    ...
    void BiAIT::updateValidQueueIfVertexInside(const std::shared_ptr<Vertex> &vertex) {
        // This vertex now has a changed heuristic. All edges in the F&R valid queue that have this vertex as a child
        // must be updated. This can not be delayed, as whether the lazy search can be suspended depends on the best
        // edge in the valid queue;
        updateFVValidQueueIfVertexInside(vertex);
        updateRVValidQueueIfVertexInside(vertex);
    }


    void BiAIT::updateFVValidQueueIfVertexInside(const std::shared_ptr<Vertex> &vertex) {
        for(const auto & elem : vertex->validQueueIncomingLookupFromStart_){
            elem->data.setEdgeKey(computeForwardEdgeKey(optObjPtr_, elem->data.getParent(), elem->data.getChild()));
            forwardValidQueue_.update(elem);
        }
    ...
    void BiAIT::updateRVValidQueueIfVertexInside(const std::shared_ptr<Vertex> &vertex) {
        for(const auto & elem : vertex->validQueueIncomingLookupFromGoal_){
            elem->data.setEdgeKey(computeReverseEdgeKey(optObjPtr_, elem->data.getParent(), elem->data.getChild()));
            reverseValidQueue_.update(elem);
        }
    ...
    biait::Queuetypes::MeetLazyEdgeQueue::Element * BiAIT::findInMeetLazyQueue(const Edge & edge) {
        ...
        auto found = std::find_if(meetLazyEdges.begin(), meetLazyEdges.end(),
                                  [parentId, childId](const auto & elem){ return
                                      (elem.getParent().lock()->getId() == parentId && elem.getChild().lock()->getId() == childId) ||
                                      (elem.getParent().lock()->getId() == childId && elem.getChild().lock()->getId() == parentId);});
        // `edge` is not in the `meetLazyEdgeQueue_`, and return nullptr;
        if(found == meetLazyEdges.end()) return nullptr;
        // Else, we try to figure out the pointer to the meetLazyEdge in `meetLazyEdgeQueue_`;
        else{
            auto parentPointerToElemVector = found->getParent().lock()->meetLazyEdgeQueuePointer_;
            auto childPointerToElemVector = found->getChild().lock()->meetLazyEdgeQueuePointer_;
            for(const auto parentPointerToElem: parentPointerToElemVector){
                if(std::find_if(childPointerToElemVector.begin(), childPointerToElemVector.end(),
                                [parentPointerToElem](const auto elem){ return parentPointerToElem == elem; })
    ...
    biait::Queuetypes::MeetValidEdgeQueue::Element * BiAIT::findInMeetValidQueue(const Edge & edge) {
        ...
        auto found = std::find_if(meetValidEdges.begin(), meetValidEdges.end(),
                                  [parentId, childId](const auto & elem){ return
                                          (elem.second.getParent()->getId() == parentId && elem.second.getChild()->getId() == childId) ||
                                          (elem.second.getParent()->getId() == childId && elem.second.getChild()->getId() == parentId);});
        // `edge` is not in the `meetValidEdgeQueue_`, and return nullptr;
        if(found == meetValidEdges.end()) return nullptr;
        // Else, we try to figure out the pointer to the meetValidEdge in `meetValidEdgeQueue_`;
        else{
            auto parentPointerToElemVector = found->second.getParent()->meetValidEdgeQueuePointer_;
            auto childPointerToElemVector = found->second.getChild()->meetValidEdgeQueuePointer_;
            for(const auto parentPointerToElem: parentPointerToElemVector){
                if(std::find_if(childPointerToElemVector.begin(), childPointerToElemVector.end(),
                                [parentPointerToElem](const auto elem){ return parentPointerToElem == elem; })
                   != childPointerToElemVector.end()){
            ...
            throw(std::logic_error("Found `edge` in `meetValidEdgeQueue_`, but the found edge's parent and child do not correct pointer to the `meetValidEdgeQueue_::Element*`;"));
        }
    }
    ...
    void BiAIT::removeFromMeetValidQueue(const std::vector<Queuetypes::MeetValidEdgeQueue::Element *>& vectorOfMeetValidEdgesToBePruned){
        for(const auto & elem: vectorOfMeetValidEdgesToBePruned){
            ...
            Utility::removeFromVectorByValue(elem->data.second.getParent()->meetValidEdgeQueuePointer_, elem);
            Utility::removeFromVectorByValue(elem->data.second.getChild()->meetValidEdgeQueuePointer_, elem);
...
//        assert(candidateParent->getCategory()[3] && candidateChild->getCategory()[0]);
        if(candidateParent->getCategory()[4] && candidateChild->getCategory()[1]){
            ...
            if(candidateParent->hasWhitelistedChild(candidateChild) ||
                motionValidatorPtr_->checkMotion(candidateParent->getState(), candidateChild->getState())){
                // Collision free!
                if(!candidateParent->hasWhitelistedChild(candidateChild)) {
                    candidateParent->whitelistAsChild(candidateChild);
                    ++numEdgeCollisionChecks_;
                }
                candidateParent->setCategory(2, true); candidateChild->setCategory(2, true);
                const auto edgeCost = optObjPtr_->motionCost(candidateParent->getState(), candidateChild->getState());
                if(optObjPtr_->isCostBetterThan(optObjPtr_->combineCosts(candidateParent->costToGoal_, edgeCost),
                                                candidateParent->costToGoal_)){
                    candidateParent->costToGoal_ = optObjPtr_->combineCosts(candidateChild->costToGoal_, edgeCost);
                }
                if(optObjPtr_->isCostBetterThan(optObjPtr_->combineCosts(candidateChild->costToStart_, edgeCost),
                                                candidateChild->costToStart_)){
                    candidateChild->costToStart_ = optObjPtr_->combineCosts(candidateParent->costToStart_, edgeCost);
                }

                Edge meetValidEdge(candidateParent, candidateChild, 0b00100,
                                   {{optObjPtr_->infiniteCost(), optObjPtr_->infiniteCost(), optObjPtr_->infiniteCost()}});
                insertOrUpdateMeetValidEdge(meetValidEdge, edgeCost);
            } else {
                ...
                Utility::removeFromVectorByValue(candidateParent->meetLazyEdgeQueuePointer_, meetLazyEdgePointer);
                if(optObjPtr_->isCostEquivalentTo(
                        candidateParent->heuristicCost_g_ReverseLazy_,
                        optObjPtr_->combineCosts(candidateChild->heuristicCost_g_ReverseLazy_,
                                                 optObjPtr_->motionCostHeuristic(candidateChild->getState(),
                                                                                 candidateParent->getState())))) {
                    invalidateFLWhenInvalidatingRL(candidateParent, nullptr);
                }
                Utility::removeFromVectorByValue(candidateChild->meetLazyEdgeQueuePointer_, meetLazyEdgePointer);
                if(optObjPtr_->isCostEquivalentTo(
                        candidateChild->heuristicCost_g_ForwardLazy_,
                        optObjPtr_->combineCosts(candidateParent->heuristicCost_g_ForwardLazy_,
                                                 optObjPtr_->motionCostHeuristic(candidateParent->getState(),
