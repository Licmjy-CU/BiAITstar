...
ImplicitGraph::ImplicitGraph(const ompl::base::Cost &solutionCost_)
  ...
  // verticesGNAT_ default param: degree = 8, minDegree = 4, maxDegree = 12,
  // maxNumPtsPerLeaf = 50, removedCacheSize = 500, rebalancing = false,
  // (estimatedDimension = 6.0);
}

void ImplicitGraph::setup(
    base::SpaceInformationPtr &spaceInformationPtr,
  ...
  verticesGNAT_.setDistanceFunction([this](const std::shared_ptr<Vertex> &a,
                                           const std::shared_ptr<Vertex> &b) {
    return spaceInformationPtr_->distance(a->getState(), b->getState());
  });

  k_RGG_ = static_cast<std::size_t>(
      boost::math::constants::e<double>() +
      boost::math::constants::e<double>() /
          static_cast<double>(spaceInformationPtr_->getStateDimension()));

  updateStartAndGoalStates(ompl::base::plannerAlwaysTerminatingCondition(),
                           plannerInputStatesPtr);
}
  ...
  for (const auto &elem : verticesToBePruned) {
    ...
    if (!elem->meetValidEdgeQueuePointer_.empty()) {
      for (const auto meetValidEdgePtr : elem->meetValidEdgeQueuePointer_) {
        if (meetValidEdgePtr != nullptr &&
            !Utility::containsByValue(vectorOfMeetValidEdgesToBePruned_,
                                      meetValidEdgePtr)) {
          Utility::removeFromVectorByValue(
              meetValidEdgePtr->data.second.getChild()
                  ->meetValidEdgeQueuePointer_,
              meetValidEdgePtr);
          Utility::removeFromVectorByValue(
              meetValidEdgePtr->data.second.getParent()
                  ->meetValidEdgeQueuePointer_,
              meetValidEdgePtr);
...
void ImplicitGraph::registerStartState(
    const base::State *const startStateCPtr) {
  auto startVertexPtr = std::make_shared<Vertex>(
      spaceInformationPtr_, problemDefinitionPtr_, batchId_);
...
void ImplicitGraph::registerGoalState(const base::State *const goalStateCPtr) {
  auto goalVertexPtr = std::make_shared<Vertex>(
      spaceInformationPtr_, problemDefinitionPtr_, batchId_);
...
void ImplicitGraph::updateStartAndGoalStates(
  ...
  // First update the goals. We have to call
  // inputStates->nextGoal(terminationCondition) at least once (regardless of
  // the return value of inputStates->moreGoalStates()) in case the termination
  // condition wants us to wait for a goal.
  do {
  ...
  } while (plannerInputStatesPtr->haveMoreGoalStates() &&
           goalVertices_.size() <= maxNumberOfGoals_);
  ...
  // If added a new goal and have previously pruned start states, try to revive
  // the start;
  if (addedNewGoalState && !prunedStartVertices_.empty()) {
    std::vector<std::vector<std::shared_ptr<Vertex> >::iterator> revivedStarts;
    for (auto it = prunedStartVertices_.begin();
         it != prunedStartVertices_.end(); ++it) {
      auto heuristicCost = optObjPtr_->infiniteCost();
  ...
  if (addedNewGoalState || addedNewStartState) {
    if (!startVertices_.empty() && !goalVertices_.empty()) {
      informedSamplerPtr_ = optObjPtr_->allocInformedStateSampler(
          problemDefinitionPtr_, std::numeric_limits<unsigned int>::max());
    }
  }
...
bool ImplicitGraph::addSamples(
    std::size_t numNewSamples,
    const base::PlannerTerminationCondition &terminationCondition) {
  if (numNewSamples == 0) return true;
  newSamples_.reserve(numNewSamples);
  do {
    newSamples_.emplace_back(std::make_shared<Vertex>(
        spaceInformationPtr_, problemDefinitionPtr_, batchId_));
    do {
      informedSamplerPtr_->sampleUniform(newSamples_.back()->getState(),
                                         solutionCost_);
      ++numSampledStates_;
    } while (!spaceInformationPtr_->getStateValidityChecker()->isValid(
        newSamples_.back()->getState()));
    if (problemDefinitionPtr_->getGoal()->isSatisfied(
            newSamples_.back()->getState())) {
  ...
  if (newSamples_.size() == numNewSamples) {
    ...
    if (useKNearest_) {
      numNeighbors_ = computeNumberOfNeighbors(
          numSamplesInInformedSet + numNewSamples - startVertices_.size() -
          goalVertices_.size());
    } else {
      radius_ =
          computeConnectionRadius(numSamplesInInformedSet + numNewSamples -
...
std::vector<std::shared_ptr<Vertex> > ImplicitGraph::getNeighbors(
    const std::shared_ptr<Vertex> &vertex) const {
  if (vertex->hasCachedNeighbors()) {  // TODO: check the batchID, maybe this is
                                       // a bug;
...
bool ImplicitGraph::isStart(const std::shared_ptr<Vertex> &vertex) const {
  if (std::any_of(startVertices_.begin(), startVertices_.end(),
                  [&vertex](const std::shared_ptr<Vertex> &elem) {
                    return elem->getId() == vertex->getId();
                  }))
    return true;
  return false;
}
  ...
  for (const auto &elemVertex : getVertices()) {
    ...
    for (const auto &elemStart : startVertices_) {
      bestCostToRoot = optObjPtr_->betterCost(
          bestCostToRoot, optObjPtr_->motionCostHeuristic(
                              elemStart->getState(), elemVertex->getState()));
    }
    ...
    for (const auto &elemGoal : goalVertices_) {
      bestCostToOppositeRoot = optObjPtr_->betterCost(
          bestCostToOppositeRoot,
          optObjPtr_->motionCostHeuristic(elemVertex->getState(),
                                          elemGoal->getState()));
    }

    if (optObjPtr_->isCostBetterThan(
            optObjPtr_->combineCosts(bestCostToRoot, bestCostToOppositeRoot),
            solutionCost_)) {
...
std::size_t ImplicitGraph::computeNumberOfNeighbors(
    std::size_t numSamples) const {
  return std::ceil(rewireFactor_ * static_cast<double>(k_RGG_) *
                   std::log(static_cast<double>(numSamples)));
}

bool ImplicitGraph::canPossiblyImproveSolution(
    const std::shared_ptr<Vertex> &vertex) const {
  // TODO: why dont we store the bestCostToRoot in the Vertex class;
  auto bestCostToRoot = optObjPtr_->infiniteCost();
  for (const auto &elemStart : startVertices_) {
    bestCostToRoot = optObjPtr_->betterCost(
        bestCostToRoot, optObjPtr_->motionCostHeuristic(elemStart->getState(),
                                                        vertex->getState()));
  }
  ...
  return optObjPtr_->isCostBetterThan(
      optObjPtr_->combineCosts(
          bestCostToRoot,
          optObjPtr_->costToGo(vertex->getState(),
                               problemDefinitionPtr_->getGoal().get())),
      solutionCost_);
}
}  // namespace ompl::geometric::biait
